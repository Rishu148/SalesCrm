...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-[120] flex items-center gap-3 px-5 py-3.5 rounded-xl border shadow-2xl backdrop-blur-xl animate-in slide-in-from-right-10 duration-300 ${
        type === "success"
          ? "bg-[#0A0A0C]/95 border-emerald-500/20 text-emerald-400"
          : "bg-[#0A0A0C]/95 border-rose-500/20 text-rose-400"
      }`}
    >
      <div className={`p-1 rounded-full ${type === "success" ? "bg-emerald-500/10" : "bg-rose-500/10"}`}>
...
const Contacts = () => {
  ...
  const getSelectableLeads = () => filteredLeads.filter((l) => l.status !== "Closed");
    ...
    <div className="min-h-screen bg-[#030303] text-slate-300 font-sans selection:bg-indigo-500/30 scroll-smooth">
          ...
          <div className="max-w-[1600px] mx-auto flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
            <div>
              ...
              <p className="text-slate-500 mt-1.5 text-xs flex items-center gap-3 font-medium tracking-wide">
                <span className="flex items-center gap-1.5"><LayoutGrid size={12} /> {leads.length} Leads</span>
            ...
            <div className="flex gap-3">
                 {user?.role === "admin" && (
                    <button
                        ...
                        viewUnassignedOnly
                            ...
                            : "bg-[#050505] text-slate-400 border-white/10 hover:border-white/20 hover:text-white"
                        }`}
                    >
                        <UserX size={14} />
                ...
                <div className="relative w-48 group">
                    ...
                    <select
                        ...
                        // FIX: Added [&>option] classes to style the dropdown options dark
        ...
        <div className="bg-[#0A0A0C]/60 border border-white/5 rounded-xl overflow-hidden shadow-sm relative z-0">
            ...
            <table className="w-full text-left text-sm border-collapse">
                ...
                <tr className="bg-[#050505] text-slate-500 border-b border-white/5 text-[10px] uppercase tracking-wider font-semibold">
                    ...
                    {user?.role === "admin" && <th className="px-6 py-4 font-medium">Agent</th>}
                ...
                {loading ? (
                    ...
                    <tr>
                    <td colSpan="8" className="text-center py-20 text-slate-600">
                    ...
                    return (
                        ...
                        onClick={(e) => {
                            ...
                            if (user?.role === "admin") toggleRowSelection(lead._id);
                        }}
                        className={`group transition-all duration-200 border-l-[3px] border-l-transparent ${
                            isClosed
                            ? "bg-white/[0.01] opacity-50 grayscale cursor-not-allowed" 
                            : isSelected
                                ? "bg-indigo-500/10 border-l-indigo-500 cursor-pointer"
                                : "hover:bg-white/[0.02] hover:border-l-indigo-500/50 cursor-pointer"
                        }`}
                        >
                        {user?.role === "admin" && (
                        ...
                        <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold border ${
                                    ...
                                    : "bg-[#0F0F12] text-slate-400 border-white/10 group-hover:border-white/20"
                                }`}>
                                    {getInitials(lead.name)}
                                </div>
                                <p className={`font-medium text-sm transition-colors ${isSelected ? "text-indigo-300" : "text-slate-200 group-hover:text-white"}`}>
                                    {lead.name}
                                </p>
                            </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                            ...
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wide border ${
                                isClosed ? "border-slate-800 text-slate-700 bg-transparent" : getStatusStyle(lead.status)
                            }`}
                            >
                            {lead.status}
                            </span>
                        ...
                        <td className="px-6 py-4 text-right">
                            {!isClosed ? (
                                // FIX: Removed opacity-0 to make them always visible. Added slight dimming.
                                <div className="flex justify-end gap-1.5 transition-opacity">
                                    <a href={`tel:${lead.phone}`} className="p-1.5 bg-[#0A0A0C] border border-white/10 rounded-lg hover:bg-blue-600 hover:text-white hover:border-blue-500 text-slate-500 transition-all cursor-pointer"><Phone size={14} /></a>
                                    <a href={`https://wa.me/${lead.phone}`} target="_blank" rel="noreferrer" className="p-1.5 bg-[#0A0A0C] border border-white/10 rounded-lg hover:bg-emerald-500 hover:text-white hover:border-emerald-500 text-slate-500 transition-all cursor-pointer"><MessageCircle size={14} /></a>
                                    
                                    {lead.email && (
                                        <a href={`mailto:${lead.email}`} className="p-1.5 bg-[#0A0A0C] border border-white/10 rounded-lg hover:bg-rose-500 hover:text-white hover:border-rose-500 text-slate-500 transition-all cursor-pointer">
                                            <Mail size={14} />
                                        </a>
                                    )}
                                </div>
                            ) : (
                                <div className="flex justify-end opacity-20"><Lock size={14}/></div>
                            )}
                        </td>
                        </tr>
                    );
      ...
      {selectedLeads.length > 0 && user?.role === "admin" && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-[#0F0F12] border border-white/10 shadow-2xl rounded-xl px-4 py-2 flex items-center gap-4 animate-in slide-in-from-bottom-6 z-[100]">
          ...
          <button onClick={() => setShowAssignModal(true)} className="flex items-center gap-2 bg-white text-black px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all active:scale-95 shadow-lg cursor-pointer">
            {isReassigning ? <RefreshCw size={14} /> : <Users size={14} />} {isReassigning ? "Reassign" : "Assign"}
          </button>
          
          <button onClick={() => setShowDeleteModal(true)} className="text-rose-400 hover:text-rose-300 font-medium text-sm flex items-center gap-2 hover:bg-rose-500/10 px-3 py-2 rounded-lg transition-colors cursor-pointer">
            <Trash2 size={16} /> Delete
          </button>
          
          <button onClick={() => setSelectedLeads([])} className="ml-2 text-slate-400 hover:text-white cursor-pointer"><X size={18} /></button>
        </div>
      )}

      {/* === MODALS === */}
      
      {/* 1. QUICK SELECT (Backdrop click closes modal) */}
      {showQuickSelectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in" onClick={() => setShowQuickSelectModal(false)}>
          <div className="bg-[#0A0A0C] border border-white/10 w-full max-w-sm rounded-2xl p-6 shadow-2xl shadow-amber-900/10 relative animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowQuickSelectModal(false)} className="absolute top-4 right-4 text-slate-500 hover:text-white cursor-pointer bg-white/5 rounded-full p-1"><X size={18} /></button>
            ...
            <div className="space-y-5">
                <div>
                    <label className="text-[10px] text-amber-400/80 font-bold uppercase tracking-wider mb-2 block">Target Group</label>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1.5 text-right">
                        {getLeadsByQuickFilter().length} leads matching criteria
                    </p>
                </div>

                <div>
                    <label className="text-[10px] text-amber-400/80 font-bold uppercase tracking-wider mb-2 block">Count</label>
                    <input
                        type="number"
                        min="1"
                        max={getLeadsByQuickFilter().length}
                        className="w-full bg-[#050505] border border-white/10 text-white p-3 rounded-xl focus:border-amber-500 focus:outline-none font-mono text-center text-lg font-bold"
                        value={quickCount}
                ...
                >
                <CheckCircle size={18} /> Select Leads
                </button>
            </div>
          </div>
        </div>
      ...
      {/* 2. Assign Modal */}
      {showAssignModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setShowAssignModal(false)}>
          <div className="bg-[#0A0A0C] border border-white/10 w-full max-w-sm rounded-2xl p-6 shadow-2xl relative animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowAssignModal(false)} className="absolute top-4 right-4 text-slate-500 hover:text-white cursor-pointer"><X size={18} /></button>
            <h3 className="text-lg font-bold text-white mb-1">Assign Agent</h3>
            <p className="text-slate-500 text-xs mb-4">Assigning {selectedLeads.length} leads.</p>
      ...
      {/* 3. Add Lead Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in" onClick={() => setShowAddModal(false)}>
          <div className="bg-[#0A0A0C] p-6 rounded-2xl border border-white/10 w-full max-w-lg shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
             <button onClick={() => setShowAddModal(false)} className="absolute top-5 right-5 text-slate-500 hover:text-white cursor-pointer"><X size={20} /></button>
             <h2 className="text-white font-bold text-lg mb-6 flex items-center gap-2"><Plus size={20} className="text-indigo-500"/> Add New Contact</h2>
      ...
      {/* 4. Delete Modal */}
      {showDeleteModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in" onClick={() => setShowDeleteModal(false)}>
          <div className="bg-[#0A0A0C] border border-red-500/20 w-full max-w-xs rounded-2xl p-6 shadow-2xl relative animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-white mb-2 text-center">Delete Leads?</h3>
            <p className="text-slate-500 text-xs mb-6 text-center">
              Are you sure you want to delete {selectedLeads.length} items? This cannot be undone.
            </p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteModal(false)} className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm font-medium cursor-pointer">Cancel</button>
              <button onClick={confirmDelete} className="flex-1 py-2 bg-red-600 hover:bg-red-500 text-white rounded-lg text-sm font-bold cursor-pointer">Delete</button>
            </div>
          </div>
        </div>
      )}

      {/* 5. IMPORT MODAL (Enhanced UI & Click Outside) */}
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in" onClick={() => setShowImportModal(false)}>
          <div className="bg-[#0A0A0C] p-8 rounded-3xl border border-white/10 w-full max-w-md shadow-2xl relative overflow-hidden" onClick={(e) => e.stopPropagation()}>
             
             {/* Decorative Background */}
             <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-[50px] pointer-events-none"></div>

             <button onClick={() => setShowImportModal(false)} className="absolute top-5 right-5 text-slate-500 hover:text-white cursor-pointer"><X size={20} /></button>
             ...
             <div className="relative z-10">
                ...
                <p className="text-slate-500 text-xs mb-6 ml-11">Add multiple contacts via Excel/CSV.</p>
                ...
                <div className="bg-[#050505] p-4 rounded-xl border border-white/5 flex justify-between items-center group hover:border-white/10 transition-all">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 font-bold text-xs border border-white/5">1</div>
                        <div>
                            <h4 className="text-sm font-bold text-white">Download Template</h4>
                            <p className="text-[10px] text-slate-500">Get the correct format.</p>
                        </div>
                    </div>
                    <button onClick={downloadSample} className="text-xs font-bold text-indigo-400 hover:text-indigo-300 cursor-pointer flex items-center gap-1 bg-indigo-500/10 px-3 py-1.5 rounded-lg border border-indigo-500/20 transition-all">
                ...
                >
                    <input type="file" ref={fileInputRef} hidden accept=".csv, .xlsx" onChange={handleFileUpload} />
                    
                    {uploading ? (
                    <div className="flex flex-col items-center gap-2">
                        <Loader2 size={32} className="text-emerald-500 animate-spin" />
                        <span className="text-xs text-emerald-400 font-bold">Processing Data...</span>
                    </div>
                    ) : (
                    <>
                        <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform shadow-lg">
                            <FileUp size={20} className="text-slate-400 group-hover:text-emerald-400" />
                        </div>
                        <p className="text-sm text-white font-bold">Click to Upload File</p>

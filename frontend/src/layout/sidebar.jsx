  ...
  return (
    <>
      ...
      >
        ...
        {/* 🟢 TOGGLE BUTTON (Perfectly Positioned) */}
        <button 
        ...
        <div className={`flex items-center gap-4 p-6 mb-4 ${isCollapsed ? "justify-center px-0" : ""}`}>
          ...
          <div 
            className={`overflow-hidden transition-all duration-300 cursor-pointer ${isCollapsed ? "w-0 opacity-0" : "w-auto opacity-100"}`}
            onClick={() => setIsCollapsed(!isCollapsed)}
          >
            <h1 className="font-bold text-white text-xl tracking-tight leading-none group-hover:text-indigo-400 transition-colors">SalesCRM</h1>
              ...
              <div key={item.name} className="relative group">
                  ...
                  >
                      ...
                      <Icon 
                        ...
                        className={`shrink-0 transition-all duration-300 relative z-10 ${active ? "text-indigo-400 drop-shadow-[0_0_8px_rgba(99,102,241,0.6)]" : "group-hover:text-slate-200"}`} 
                      />
        ...
        <div className="p-4 border-t border-white/5 bg-[#050505]">
          ...
          <div
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl text-slate-500 hover:bg-white/5 hover:text-white transition-all mb-2 cursor-pointer group relative ${isCollapsed ? "justify-center" : "pl-4"}`}
          >
          ...
          <div className={`flex items-center gap-3 p-3 rounded-xl bg-[#0A0A0C] border border-white/5 transition-all duration-300 hover:border-white/10 ${isCollapsed ? "justify-center bg-transparent border-0 p-0" : ""}`}>
              <div className="relative shrink-0 cursor-pointer hover:scale-105 transition-transform">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-tr from-indigo-500/10 to-purple-500/10 flex items-center justify-center text-indigo-400 font-bold border border-white/10">
              ...
              <div className={`overflow-hidden transition-all duration-300 ${isCollapsed ? "w-0 opacity-0 hidden" : "w-auto opacity-100 block"}`}>
                  ...
                  <p className="text-[10px] text-slate-500 truncate max-w-[120px] font-mono">{user?.email}</p>
              </div>

              {!isCollapsed && (
                  <button 
                    onClick={handleLogout} 
                    className="ml-auto p-2 text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors cursor-pointer" 
                    title="Logout"
                  >
                      <LogOut size={16} />
                  </button>
